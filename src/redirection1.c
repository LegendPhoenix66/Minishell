...
int handle_redirection(t_list *token)
{
    if (strcmp(token->content, ">") == 0)
        return handle_output_redir(token->next->content, O_WRONLY | O_CREAT | O_TRUNC);
    else if (strcmp(token->content, ">>") == 0)
        return handle_output_redir(token->next->content, O_WRONLY | O_CREAT | O_APPEND);
   /* else if (strcmp(token->content, "<") == 0)
        return handle_input_redir(token->next->content);
    else if (strcmp(token->content, "<<") == 0)
        return handle_heredoc(token->next->content);*/
    return -1;
}
